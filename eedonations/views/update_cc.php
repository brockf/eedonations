<? if ($errors) { ?>
	<div class="eedonations_error"><?=$errors;?></div>
<? } ?>
...
$this->table->set_heading(
					array('data' => lang('eedonations_create_subscription'), 'colspan' => '2')
						);
...
$this->table->add_row(
			...
			$config['intervals'][$subscription['interval']] . ' (' . $subscription['interval'] . ' days)'
		);
...
$this->table->add_row(
		lang('eedonations_order_form_customer_country'),
		form_dropdown('country', $countries, $address['country'])
	);
	
$this->table->add_row(
		lang('eedonations_order_form_customer_postal_code'),
		form_input(array('name' => 'postal_code', 'value' => $address['postal_code'], 'style' => 'width: 250px'))
	);
	
$this->table->add_row(
		'',
		form_submit('submit_form', $this->lang->line('eedonations_update_cc_button'))
	);
