...
class EEDonations_upd {
	...
	function update ($current = '') {
		if ($current < '1.01') {
			$this->EE->db->query('ALTER TABLE `exp_eedonations_payments` ADD COLUMN `member_id` INT(11) AFTER `charge_id`');
		}

		if ($current < '1.02') {
			$this->EE->db->query('ALTER TABLE `exp_eedonations_payments` MODIFY COLUMN `member_id` VARCHAR(32)');
			$this->EE->db->query('ALTER TABLE `exp_eedonations_subscriptions` MODIFY COLUMN `member_id` VARCHAR(32)');
			$this->EE->db->query('ALTER TABLE `exp_eedonations_address_book` MODIFY COLUMN `member_id` VARCHAR(32)');
		}

		if ($current < '1.09') {
			$this->EE->db->query('CREATE TABLE `exp_eedonations_custom_fields` (
									  `custom_field_id` int(11) unsigned NOT NULL auto_increment,
									  `member_id` int(11) default NULL,
									  `payment_id` int(11) default NULL,
									  `recurring_id` int(11) default NULL,
									  `custom_field_name` varchar(250) NULL,
									  `custom_field_value` varchar(250) NULL,
									  PRIMARY KEY  (`custom_field_id`)
									) ENGINE=MyISAM DEFAULT CHARSET=utf8;');
		}

		if ($current < '1.10') {
			$this->EE->db->query('CREATE TABLE `exp_eedonations_countries` (
			...
			$this->EE->db->query("INSERT INTO `exp_eedonations_countries` (`country_id`, `iso2`, `iso3`, `name`, `available`)
									VALUES
	...
	function install () {
        ...
        $sql[] = "CREATE TABLE IF NOT EXISTS `exp_eedonations_config` (
				  ...
				  `allow_custom_intervals` TINYINT(1) NOT NULL,
				  `allow_custom_amounts` TINYINT(1) NOT NULL,
				  `minimum_amount` FLOAT NOT NULL,
				  `maximum_amount` FLOAT NOT NULL
				  ) ENGINE = MyISAM DEFAULT CHARSET=utf8;";

		$sql[] = "CREATE TABLE IF NOT EXISTS `exp_eedonations_subscriptions` (
		...
		$sql[] = "CREATE TABLE IF NOT EXISTS `exp_eedonations_payments` (
		...
		$sql[] = "CREATE TABLE IF NOT EXISTS `exp_eedonations_address_book` (
		...
		$sql[] = 'CREATE TABLE `exp_eedonations_custom_fields` (
					  `custom_field_id` int(11) unsigned NOT NULL auto_increment,
					  `member_id` int(11) default NULL,
					  `payment_id` int(11) default NULL,
					  `recurring_id` int(11) default NULL,
		...
		$sql[] = 'CREATE TABLE `exp_eedonations_countries` (
								  `country_id` int(11) NOT NULL,
								  `iso2` varchar(2) NOT NULL,
								  `iso3` varchar(3) NOT NULL,
								  `name` varchar(255) NOT NULL,
		...
		$sql[] = "INSERT INTO `exp_eedonations_countries` (`country_id`, `iso2`, `iso3`, `name`, `available`)
									VALUES
								(4,'AF','AFG','Afghanistan','1'),
								(248,'AX','ALA','Aland Islands','1'),
								(8,'AL','ALB','Albania','1'),
		...
		$sql[] = 'INSERT INTO `exp_eedonations_countries` (`country_id`, `iso2`, `iso3`, `name`)
				VALUES
					(895, \'CW\', \'CW\', \'Curaçao\'),
	...
	function uninstall () {
        ...
        $sql[] = "DELETE FROM `exp_modules` WHERE `module_name`='EEDonations'";
        $sql[] = "DROP TABLE `exp_eedonations_config`";
        $sql[] = "DROP TABLE `exp_eedonations_subscriptions`";
        $sql[] = "DROP TABLE `exp_eedonations_address_book`";
        $sql[] = "DROP TABLE `exp_eedonations_payments`";
        $sql[] = 'DELETE FROM `exp_actions` WHERE `class` = \'EEDonations\'';
